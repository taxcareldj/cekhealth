...
.hero::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 60% 40% at 50% -5%,rgba(26,86,255,.07),transparent);pointer-events:none;}
...
/* ── simulator-only functions (safe if element missing) ── */
function fmtCoa(el){if(!el)return;const v=parseN(el.value);el.value=v>0?fmtN(v):'0'}
function rawCoa(el){if(!el)return;const v=parseN(el.value);if(v>0)el.value=v}
function fmtOmzet(){
...
function onOmzetInput(){
  const err=document.getElementById('omzet-err');
...
function applyPreset(key,btn){
  ...
  if(!omzetEl){return;}
  const omzet=parseN(omzetEl.value);
  if(!omzet){const err=document.getElementById('omzet-err');if(err)err.textContent='Isi omzet dulu';return}
  const d=PRESETS[key];
  ...
  const atg=document.querySelector('.atg');
  if(acb)acb.classList.add('open');
  if(atg)atg.textContent='sembunyikan \u25b2';
  updateTotal();
}
...
function doSubmit(){
  ...
  if(v<1000000){
    const err=document.getElementById('omzet-err');
    if(err)err.textContent='Omzet minimal Rp 1.000.000';
    omzetEl.focus();
    return;
  }
  omzetEl.value=v;
